...
<body>
    ...
    <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
        ...
        <form class="d-flex" method="GET" action="actionuser.php">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="row">
                    ...
                    if (isset($_GET['search']) && $_GET['search'] != '') {
                        ...
                        $query = "SELECT `id`, `carId`, `username`, `action`, `Time` FROM `log_entry`where `id` LIKE '$searchQuery' OR `action` LIKE '$searchQuery' OR `Time` LIKE '$searchQuery' ORDER BY `Time` ASC ";
                    } else {
                        $query = "SELECT `id`, `carId`, `username`, `action`, `Time` FROM `log_entry`ORDER BY `Time` ASC ";
                    }
                    ...
                    if ($result) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo '<div ><b>' . $row['id'] . ' - ' . $row['username'] . '</b></div>';
                            echo '</div>';
                            echo '<div class="card-body">';
                            echo '<h5 class="card-title text-primary">' . $row['Time'] . '</h5>';
                            echo '<p class="card-text text-primary">' . $row['action'] . ' </p>';
